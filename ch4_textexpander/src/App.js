...
function TextExpander({
	...
	const [nwords, setnwords] = useState(collapsedNumWords);

	function handleClick() {
		setshowMore(!showMore);
		if (!showMore) setnwords(children.length);
		else setnwords(collapsedNumWords);
	}
	...
	return (
		<div className={className}>
			{showMore ? (
				<div>
					<span>{children.slice(0, nwords)} </span>
					<button
						onClick={handleClick}
						style={{
							color: buttonColor,
